...
export const AddItemModal: React.FC<AddItemModalProps> = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      title={t("addItemModal.title")}
      onClose={handleClose}
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex flex-col gap-2">
          <label className="block text-sm font-medium text-gray-900">
            {t("addItemModal.itemType")}
          </label>
          <div className="flex gap-4">
            ...
            >
              <input
                type="radio"
                value="material"
                checked={itemType === ItemType.MATERIAL}
                onChange={(e) => setItemType(e.target.value as ItemType)}
                disabled={isLoading}
                className="cursor-pointer"
              />
              <span className="text-sm font-medium">
                {t("addItemModal.material")}
              </span>
            </label>
            <label
              className={`flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer transition-all ${
          ...
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                placeholder={t("addItemModal.pricePlaceholder")}
                disabled={isLoading}
                className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-gray-100 disabled:text-gray-600"
              />
            </div>
